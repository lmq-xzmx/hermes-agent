...
export function useContextMenu() {
  ...
  const target = ref(null) // 当前操作的目标文件
  const menuItems = ref([...DEFAULT_ITEMS])
   ...
   * @param {Object} fileItem - 目标文件项
   * @param {Array} customItems - 自定义菜单项（可选）
   */
  function show(event, fileItem = null, customItems = []) {
   ...
   * 切换菜单可见性
   */
  function toggle(event, fileItem = null) {
    if (visible.value) {
      hide()
    } else {
      show(event, fileItem)
   ...
   * @returns {Object} 操作结果 { handled: boolean, data?: any }
   */
   ...
   * 创建带危险标记的菜单项（用于删除操作）
   */
  function createDangerItem(item) {
    return {
      ...item,
      danger: true
    }
