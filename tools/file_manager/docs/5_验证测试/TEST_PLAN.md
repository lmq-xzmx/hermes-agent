...
| TC ID | 测试用例 | 状态 | 验证文件 |
|-------|---------|------|----------|
| TC-UI9-001 | 拖拽文件到目标文件夹，高亮显示可释放区域 | ✅ 已完成 | useDragMove.ts |
| TC-UI9-002 | 拖拽过程中按 Escape 取消 | ✅ 已完成 | useDragMove.ts |
| TC-UI9-003 | 拖拽到无效区域自动弹回 | ✅ 已完成 | useDragMove.ts |
| TC-UI9-004 | 批量拖拽多个文件 | ✅ 已完成 | useDragMove.ts |
| TC-UI9-005 | 拖拽权限检查（非所有者不可移动） | ✅ 已完成 | useDragMove.ts |

> **实现说明**: `useDragMove.ts` composable 实现了完整的拖拽移动功能，包括:
> - `registerDragSource()` 注册拖拽源
> - `registerDropTarget()` 注册拖拽目标
> - `handleDragEnter/Over/Leave/Drop` 拖拽事件处理
> - `checkPermission` 权限检查回调
> - `Escape` 取消拖拽
> - 拖拽预览元素

### 5.7 Sidebar 导航 E2E 测试

| TC ID | 测试用例 | 状态 | 验证方式 |
|-------|---------|------|----------|
| TC-SB-001 | Sidebar 正确渲染 6 个导航项 | ✅ 已完成 | 视觉验证 |
| TC-SB-002 | 点击导航项切换 active 状态 | 🔄 需 E2E | sidebar.spec.ts |
| TC-SB-003 | 当前视图高亮显示 | 🔄 需 E2E | sidebar.spec.ts |
| TC-SB-004 | 用户名正确显示 | ✅ 已完成 | 视觉验证 |
| TC-SB-005 | 退出按钮触发 logout 事件 | 🔄 需 E2E | sidebar.spec.ts |
| TC-SB-006 | 主题按钮触发 open-theme 事件 | 🔄 需 E2E | sidebar.spec.ts |

### 5.8 知识库模块测试

| TC ID | 测试用例 | 状态 | 验证方式 |
|-------|---------|------|----------|
| TC-KB-001 | 不同权限用户访问知识模块 | 🔄 待开发 | knowledge.spec.ts |
| TC-KB-002 | 同步状态查询（llm_wiki 服务状态） | ✅ 已完成 | KnowledgeView.vue |
| TC-KB-003 | 知识库搜索功能 | ✅ 已完成 | KnowledgeView.vue |
| TC-KB-004 | 同步模式选择（manual/interval/webhook） | ✅ 已完成 | KnowledgeView.vue |
| TC-KB-005 | 空间成员同步文档到知识库 | ✅ 已完成 | server.py + knowledge_service.py |
| TC-KB-006 | 非成员无法访问知识库同步 | ✅ 已完成 | server.py + knowledge_service.py |

> **实现说明**: `KnowledgeView.vue` 组件实现了知识库管理功能，包括:
> - `checkLlmWikiStatus()` 获取 llm_wiki 服务状态
> - `syncToLlmWiki()` 同步文档到知识库
> - `searchKnowledge()` 搜索知识库
> - `showSyncSettings()` 配置同步模式

### 5.9 E2E 测试代码 (Playwright)
...
test.describe('Sidebar Navigation', () => {
  ...
  test('TC-SB-002: should highlight active nav item', async ({ page }) => {
    // 点击"团队"导航
    await page.click('.sidebar__nav-item:has-text("团队")')

    // 验证 active 状态
    const activeItem = page.locator('.sidebar__nav-item.is-active')
    await expect(activeItem).toHaveClass(/is-active/)
    await expect(activeItem).toContainText('团队')
  })

  test('TC-SB-003: should switch views on nav click', async ({ page }) => {
    // 点击各个导航项
    const navItems = [
      { text: '文件', view: 'files' },
      { text: '团队', view: 'teams' },
      { text: '空间', view: 'spaces' },
      { text: '存储池', view: 'pools' },
      { text: '知识', view: 'knowledge' },
      { text: '回收站', view: 'trash' }
    ]

    for (const item of navItems) {
      await page.click(`.sidebar__nav-item:has-text("${item.text}")`)
      await expect(page.locator('.sidebar__nav-item.is-active')).toContainText(item.text)
    }
  })

  test('TC-SB-005: should emit logout event on button click', async ({ page }) => {
    // 监听 logout 事件
    let logoutEmitted = false
    page.on('callback', (data) => {
      if (data?.event === 'logout') logoutEmitted = true
    })

    // 点击退出按钮
    await page.click('.sidebar__logout')

    // 验证 logout 事件触发（根据具体实现方式调整）
    await expect(page.locator('.sidebar__logout')).toBeVisible()
  })

  test('TC-SB-006: should emit open-theme event on theme button click', async ({ page }) => {
    // 点击主题按钮
    await page.click('.sidebar__theme-btn')

    // 验证主题面板打开或事件触发（根据具体实现调整）
    await expect(page.locator('.sidebar__theme-btn')).toBeVisible()
  })
})

test.describe('UI-9 Drag and Drop', () => {
  test('TC-UI9-001: should highlight drop target on drag over', async ({ page }) => {
    await page.goto('/files')

    // 获取文件元素和目标文件夹
    const file = page.locator('.file-item').first()
    const targetFolder = page.locator('.folder-item').first()

    // 执行拖拽
    await file.dragTo(targetFolder)

    // 验证目标高亮
    await expect(targetFolder).toHaveClass(/drag-over/)
  })

  test('TC-UI9-002: should cancel drag on Escape key', async ({ page }) => {
    await page.goto('/files')
  ...
  test('TC-UI9-004: should batch drag multiple files', async ({ page }) => {
    await page.goto('/files')
    ...
    // 执行批量拖拽
    const targetFolder = page.locator('.folder-item').first()
    const selectedFiles = page.locator('.file-item.selected')
    await selectedFiles.first().dragTo(targetFolder)
