...
> **更新说明 v1.1**: 补充第五节"契约测试"，包含 Lifecycle 契约测试、后端单元测试、前端契约测试完整示例
...
{
  "error": {
    "code": "LIFECYCLE_NO_AVAILABLE_POOL",
...
class TestLifecycleContract:
    ...
    # ========== 契约2: 引导 Action 格式 ==========
    def test_guidance_action_navigate_format(self, engine):
        """
        契约: guidance 包含 path 时，action_type 必须为 "navigate"
        """
        ...
        try:
            ...
            assert e.guidance.path is not None

    def test_guidance_action_callback_format(self, engine):
        """
        契约: guidance 包含 action(无 path) 时，action_type 必须为 "callback"
        """
        ...
        rule = ConstraintRule(
            ...
            error_message="Error",
            guidance={"label": "Contact", "action": "contact_admin"}  # 无 path
        )
...
describe('LifecycleInterceptor Contract Tests', () => {
  ...
  // ========== 契约2: guidance 格式一致性 ==========
  it('should match backend guidance format for navigate', async () => {
    ...
    expect(result.guidance).toEqual(expect.objectContaining({
      ...
      path: '/teams'  // 必须与后端 GuidanceAction.path 一致
    }))
  })
