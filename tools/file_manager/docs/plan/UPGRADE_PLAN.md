...
| | LifecycleInterceptor | `web/js/lifecycle-interceptor.js` | ✅ |
| **模块三** | GuidanceEngine (后端) | `services/guidance_engine.py` | ✅ |
| | GuidanceModal | `web/src/components/GuidanceModal.js` | ✅ |
| | CoachMark | `web/src/components/CoachMark.js` | ✅ |
| | TourGuide | `web/js/components/TourGuide.vue` | ✅ |
...
| 任务 | 说明 | 优先级 |
|------|------|--------|
| 引导状态持久化 | localStorage 存储引导完成状态 | 中 |
| 工作流引导 Tour | 工作流创建引导 Tour | 低 |
| 笔记本引导 | 笔记本协作编辑引导 | 低 |
| WebSocket 实时推送 | Admin 实时数据更新（当前轮询替代） | 中 |
...
> 📝 2026-05-02 核实调整: 模块一/三下调，WebSocket/持久化/触发点未完成