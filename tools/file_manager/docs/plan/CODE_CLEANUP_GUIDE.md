...
> **目的**: 建立严谨、安全的冗余代码清理流程
>
> **更新记录**: v1.1 - 增加"对比决策"原则，要求每个冗余项必须提供功能对比表格
...
#### 冗余项 2: 前端组件重复 (Vanilla JS vs Vue 3)
...
**分析结论**: 保留 Vue 3 版本，删除 Vanilla JS 版本（CoachMark 除外需确认无引用）。
...
### 5.1 Phase 1: 分析 (执行人: 技术负责人)

| 任务 | 负责人 | 状态 | 完成日期 |
|------|--------|------|---------|
| 分析 `services/` vs `engine/` 重复 | - | ⏳ 待执行 | - |
| 分析 `web/js/` vs `web/src/` 重复 | - | ⏳ 待执行 | - |
| 识别所有引用关系 | - | ⏳ 待执行 | - |
| 制定清理优先级 | - | ⏳ 待执行 | - |

### 5.2 Phase 2: 备份 (执行人: 全员)

| 任务 | 负责人 | 状态 | 完成日期 |
|------|--------|------|---------|
| 创建 git tag `cleanup-backup-YYYYMMDD` | - | ⏳ 待执行 | - |
| 确认所有测试通过 | - | ⏳ 待执行 | - |

### 5.3 Phase 3: 清理 (执行人: 各模块负责人)

| 任务 | 风险 | 负责人 | 状态 | 验证方式 |
|------|------|--------|------|---------|
| 删除 `engine/lifecycle_engine.py` | 🔴 | - | ⏳ | 运行 lifecycle 测试 |
| 迁移 `web/js/components/` → Vue | 🟡 | - | ⏳ | 功能测试 |
| 删除 `web/css/coach-mark.css` | 🟢 | - | ⏳ | 构建成功 |
| 删除空目录 | 🟢 | - | ⏳ | 构建成功 |
...
# 2. 前端构建
cd tools/file_manager/web && npm run build
