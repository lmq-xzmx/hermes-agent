...
>
> 状态说明: 与 `services/lifecycle_engine.py` 和 `web/src/interceptors/LifecycleInterceptor.js` 完全一致
>
> **架构更新 (2026-05-05)**: 前端已迁移至 Vue 3 SPA，使用 `vue.html` 入口，生命周期拦截器已重构为 Vue 组件
>
> **支撑文档**:
> - [ERROR_CODE_CONTRACT.md](../4_api/ERROR_CODE_CONTRACT.md) - 错误码契约与前后端映射
> - [INTERFACE_CONTRACT.md](../4_api/INTERFACE_CONTRACT.md) - REST API 与 WebSocket 消息契约
> - [lifecycle_config.yaml](../4_api/lifecycle_config.yaml) - 配置溯源说明（实际生效配置: config/lifecycle_config.yaml）
...
生命周期约束前端已从纯 JavaScript 类重构为 Vue 3 Pinia Store：

| 组件/文件 | 说明 |
|-----------|------|
| `lifecycleStore.js` | Pinia Store，封装所有约束规则和引导逻辑 |
| `useLifecycle` | 组合式函数 (composable) |
| `GuidanceModal.vue` | 引导弹窗组件 |
| `lifecycleConfig.yaml` | 后端约束配置溯源 |
...
class ErrorCode(Enum):
    ...
    HAS_BATCH_PERMISSION = "HAS_BATCH_PERMISSION"
...
> **⚠️ 重要更新 (2026-05-13)**: 以下 3.1-3.4 章节描述的 JavaScript 类实现为旧版 legacy 实现。
> **当前实现**: `web/src/stores/lifecycleStore.js` (Pinia Store)
>
> 如需了解当前实现细节，请直接阅读 `lifecycleStore.js` 源文件。
...
| transfer_quota | NOT_SPACE_OWNER | 是目标空间 Owner | "只有空间所有者可以转移配额" | 联系管理员 |
| restore_file | FILE_IN_TRASH | 文件在回收站中 | "文件不在回收站中，无法恢复" | 查看文件 |
| batch_delete | HAS_BATCH_PERMISSION | 有批量操作权限 | "您没有批量删除权限" | 联系管理员 |
