...
export const fullSyncBatchTool: ToolDefinition = {
...
interface FullSyncBatchArgs {
  task_id: string;
  batch_index: number;
  files: FileItem[];
}

export async function fullSyncBatchHandler(
  args: unknown,
  hermes: HermesClient
): Promise<ToolResult> {
  const parsed = args as FullSyncBatchArgs;

  try {
    const result = await hermes.fullSyncBatch(
      parsed.task_id,
      parsed.batch_index,
      parsed.files
    );
